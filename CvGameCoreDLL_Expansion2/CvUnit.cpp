...
bool CvUnit::isBetterDefenderThan(const CvUnit* pDefender, const CvUnit* pAttacker) const
{
	...
	if(pAttacker)
	{
		...
		{
			if(IsGreatPerson() && getGPAttackState() > -1 && (!pDefender->IsGreatPerson() || !(pDefender->getGPAttackState() > -1)/*ls612: Fixed unsafe boolean operation*/))	// this unit is Warrior GP, test defender is not
				return true;
